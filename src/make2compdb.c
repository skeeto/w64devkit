...
#define VERSION "2026-09-15" //< ISO-8601 date format
...
// Modified from Christopher Wellons pkg-config.c [1]
//
...
// Taken from Christopher Wellons pkg-config.c [1]
//
// [1] https://github.com/skeeto/w64devkit/blob/master/src/pkg-config.c
//     Licence: Unlicense
...
// We try to detect a windows path by looking for patterns that looks like "C:/" or "../"
...
static b32 str_looks_like_windows_path(Str s)
{
    isize i = str_find_any(s, SL("/\\"));
    if (i < 0) return 0;

    s = str_take_head(s, i);
    if (str_ends_with(s, SL(".."))) {
        return 1;
    }
    else if (str_ends_with(s, SL("."))) {
        return 1;
    }
    else if ((s.len >= 2) && is_ascii_alpha(s.ptr[s.len - 2]) && str_ends_with(s, SL(":"))) {
        return 1;
    }
    else {
        return 0;
...
static Str str_normalize_path(Str path)
{
    ...
    for (isize reader_idx = 0; reader_idx < path.len; reader_idx++) {
        ...
        if (c == '\\') {
            u8 next = ((reader_idx + 1) < path.len) ? path.ptr[reader_idx + 1] : 0;
            if (next == '\\') {
                reader_idx++; //< skip
            }

            if (next == ' ') {
                // <backspace><space> is converted just to space
                reader_idx++; //< skip
                path.ptr[new_len++] = ' ';
            }
            else {
...
static StrBuf strbuf_alloc(StrBuf *sb, Arena *a, isize capacity)
{
    assert(sb);
    assert(a);
    assert(capacity >= 0);
...
static isize strbuf_add(StrBuf *sb, u8 c)
{
...
static Str strbuf_finalize(StrBuf *sb, Arena *a)
{
    assert(sb);
    assert(a);
...
static Str strlist_node_delete(StrListNode *node)
{
    assert(node);
...
// Unlink `node` from the list and return its string.
// The node itself is not reclaimed: its memory belongs to the arena.
static Str strlist_unlink(StrList *sl, StrListNode *node)
{
    assert(sl);
    assert(node);
    assert(sl->count > 0);
    ...
    if (node->prev) {
        ...
        assert(sl->front == node); //< `node` was the front
        sl->front = node->next;
    }
    ...
    if (node->next) {
        ...
        assert(sl->back == node); //< `node` was the back
        sl->back = node->prev;
    }

    sl->count -= 1;
    assert((sl->count == 0) == (sl->front == NULL));

    return strlist_node_delete(node);
}
...
static Str strlist_pop_front(StrList *sl)
{
    ...
    if (strlist_is_empty(*sl)) {
        ...
        return strlist_unlink(sl, sl->front);
    }
}
...
static Str strlist_pop_back(StrList *sl)
{
    ...
    if (strlist_is_empty(*sl)) {
        ...
        return strlist_unlink(sl, sl->back);
    }
}
...
// Remove the exact slice (same memory, same length).
// rather than an equal string. Use this when several nodes can hold the same
// text but only one specific token must go.
static b32 strlist_remove_slice(StrList *sl, Str to_remove)
{
    assert(sl);

    for (StrListNode *node = sl->front; node != NULL; node = node->next) {
        if (node->str.ptr == to_remove.ptr && node->str.len == to_remove.len) {
            strlist_unlink(sl, node);
            return 1;
        }
    }
    return 0;
...
static void print_number(OsWriterInterface *w, isize number)
{
    ...
    usize uval     = negative ? ((usize)(0 - (usize)number)) : ((usize)number);
...
static Str shell_tokenize_next(Str *shell_input, b32 *eol)
{
    ...
    while (!scanner_at_end(sc)) {
        ...
        default: {
            ...
            case ' ':  // FALLTHROUGH
            case '\t': {
                // We drop the whitespace. Leading whitespace produces an empty
                // token, which the caller discards, so the next token begins at
                // scanner position 0: the redirect, comment, and continuation
                // cases above rely on that.
...
static StrList shell_tokenize_one_logical_line(Arena *perm, Str *shell_input)
{
    ...
    while (!end_of_line && (shell_input->len > 0)) {
        ...
        if (token.len > 0) {
            ...
            if (str_looks_like_windows_path(token)) {
                token = str_normalize_path(token);
            }

            Str unescape_token = shell_str_unescape(perm, token);
            strlist_push_back(&tokens, perm, unescape_token);
        }
    }
    return tokens;
}
...
static Compiler compiler_parse(Str input)
{
    ...
    while (segment.tail.len > 0) {
        ...
        if (str_equal(segment.head, SL("clang-cl"))) {
            ...
            else if (str_equal(segment.head, SL("win32")) || str_equal(segment.head, SL("posix"))) {
                // e.g. x86_64-w64-mingw32-gcc-posix
                compiler.kind   = COMPILER_IS_GCC_COMPATIBLE;
                compiler.string = input;
                return compiler;
            }
...
static GccLanguage gcc_identify_language_from_x_flag(Str x_flag)
{
    // Source: Gcc Manual
    // https://gcc.gnu.org/onlinedocs/gcc/Overall-Options.html
    static Str s_c_language[] = {
        STATIC_SL("c"),
        STATIC_SL("cpp-output"), //< The `cpp` here stands for "C Pre-Processor" '-_-
    };
...
static CompilerCommand compiler_command_from_gcc_invocation(Arena *perm, StrList tokens)
{
    ...
    while (!strlist_is_empty(tokens)) {
        ...
        else if (flag.kind == GCC_FLAG_IS_NOT_CONSUMER_FLAG) {
                                                      ...
                                                      OsWriterInterface *w, b32 verbose)
{
    ...
    // Currently `cmd.arguments` are the same for all source files.
    // But if we have more than 1 source file, we don't want to see it in the list of args.
    // e.g.
    //              "gcc", "-o", "main", "lib.c", "main.c"
    //
    // Needs to become
    //              args 1: "gcc", "-o", "main", "lib.c"
    //              args 2: "gcc", "-o", "main", "main.c"
    //
    if (compiler_command.source_files.count > 1) {
        for (CommandObject *cmd = command_objects.first; cmd != NULL; cmd = cmd->next) {
            Str source_file_to_delete = cmd->file;
            for (CommandObject *cmd_to_modify = command_objects.first; cmd_to_modify != NULL;
                 cmd_to_modify                = cmd_to_modify->next) {
                if (cmd_to_modify != cmd) {
                    strlist_remove_slice(&cmd_to_modify->arguments, source_file_to_delete);
                }
            }
        }
    }
                                       ...
                                       DirectoryStack dir_stack, b32 verbose)
{
    ...
    while (input.len > 0) {
        ...
        case PARSING_MODE_MAKE_LEAVE_DIR: {
            ...
            Str dir = directory_from_make_dir_line(&input);
            if (dir.len > 0 && dir_stack.stack.count > 1) {
                Str dir_peek = dirstack_peek(dir_stack, &scratch);
                if (str_equal(dir, dir_peek)) { // should always be true
...
static void run_test_unescape(Arena *perm, Str input, Str expected)
{
    Arena scratch = *perm;
    input         = str_copy(&scratch, input);
...
static void test_shell_tokenizer(Arena a)
{
    ...
    run_test_shell_tokenizer(a, SL("\"C:/Users/John Falstaff/w64devkit/bin/gcc.exe\" -std=c11 -g -o app main.c"),
    ...
    // Redirects and comments preceded by more than one whitespace character.
    {
        run_test_shell_tokenizer(a, SL("cmd  2>file"), SLIST(&a, "cmd", "2>", "file"));
        run_test_shell_tokenizer(a, SL("cmd\t\t2>&1"), SLIST(&a, "cmd", "2>&", "1"));
        run_test_shell_tokenizer(a, SL("gcc -O2  -c x.c  2>/dev/null"), SLIST(&a, "gcc", "-O2", "-c", "x.c", "2>", "/dev/null"));
        run_test_shell_tokenizer(a, SL("gcc main.c  # -o main"), SLIST(&a, "gcc", "main.c"));
    }

    // Escaped characters outside quotes
    {
        run_test_shell_tokenizer(a, SL("echo \\\"hello\\\""), SLIST(&a, "echo", "\"hello\""));
        run_test_shell_tokenizer(a, SL("echo \\$HOME"), SLIST(&a, "echo", "$HOME"));
        run_test_shell_tokenizer(a, SL("echo a\\\\b"), SLIST(&a, "echo", "a\\b"));
        run_test_shell_tokenizer(a, SL("echo \\ leading_space"), SLIST(&a, "echo", " leading_space"));
        run_test_shell_tokenizer(a, SL("echo say\\'hi\\'"), SLIST(&a, "echo", "say'hi'"));
    }
    ...
    // Inside double quotes, backslash only escapes $ ` " \ and newline; otherwise literal
    {
        run_test_shell_tokenizer(a, SL("echo \"a\\qb\""), SLIST(&a, "echo", "a\\qb"));
        run_test_shell_tokenizer(a, SL("echo \"a\\\"b\""), SLIST(&a, "echo", "a\"b"));
        run_test_shell_tokenizer(a, SL("echo \"a\\$b\""), SLIST(&a, "echo", "a$b"));
        run_test_shell_tokenizer(a, SL("echo \"a\\\\b\""), SLIST(&a, "echo", "a\\b"));
    }

    // Empty quotes still produce an empty-string token, not zero tokens
    {
        run_test_shell_tokenizer(a, SL("echo \"\""), SLIST(&a, "echo", ""));
        run_test_shell_tokenizer(a, SL("echo ''"), SLIST(&a, "echo", ""));
        run_test_shell_tokenizer(a, SL("echo \"\"''"), SLIST(&a, "echo", ""));
    }

    // Adjacent quoted/unquoted segments join into a single unescaped token
    {
        run_test_shell_tokenizer(a, SL("echo foo\"bar baz\"qux"), SLIST(&a, "echo", "foobar bazqux"));
        run_test_shell_tokenizer(a, SL("echo 'foo'\"bar\"baz"), SLIST(&a, "echo", "foobarbaz"));
        run_test_shell_tokenizer(a, SL("echo foo'bar'\"baz\""), SLIST(&a, "echo", "foobarbaz"));
    }

    // Unterminated quote (keep the opening quote character, rest of line becomes the token body)
    {
        run_test_shell_tokenizer(a, SL("echo \"unterminated"), SLIST(&a, "echo", "unterminated"));
        run_test_shell_tokenizer(a, SL("echo 'unterminated"), SLIST(&a, "echo", "unterminated"));
    }

    // Nested quote types within double quotes (single quotes are literal inside double quotes)
    {
        run_test_shell_tokenizer(a, SL("echo \"it's fine\""), SLIST(&a, "echo", "it's fine"));
        run_test_shell_tokenizer(a, SL("echo \"outer 'mid `inner` mid' outer\""),
                                 SLIST(&a, "echo", "outer 'mid `inner` mid' outer"));
    }
}

static void run_test_parse_dir(Arena scratch, Str input, Str expected_enter_dir, Str expected_leave_dir)
{
    input = str_copy(&scratch, input);
...
static void test_parse_directory(Arena a)
{
    ...
    run_test_parse_dir(a,
                       SL("make: Entering directory 'C:/dev/gb/make2compdb/input/simple_inline'\n"
    ...
    run_test_parse_dir(a,
                       SL("make-3.81: Entering directory 'C:/dev/gb/make2compdb/input/simple_inline'\n"
                          "gcc -std=c11 -Wall -Wextra -Wpedantic -g -o app main.c\n"
...
static void test_compiler_parse(Arena a)
{
    ...
    run_test_compiler_parser(SL("x86_64-w64-mingw32-gcc-posix"),                                       SL("x86_64-w64-mingw32-gcc-posix"), COMPILER_IS_GCC_COMPATIBLE); 
    run_test_compiler_parser(SL("x86_64-w64-mingw32-gcc-win32"),                                       SL("x86_64-w64-mingw32-gcc-win32"), COMPILER_IS_GCC_COMPATIBLE); 
...
static void test_shell_parsing(Arena a)
{
    // Simple test
    {
    ...
    // More flags
    {
        ...
        command_objects_push_back(&expected_list, &(CommandObject){
                                                      ...
                                                      .file      = SL("main.c"),
                                                      .output    = SL("main"),
                                                      .arguments = SLIST(&a, "gcc", "-std=c23", "-O0", "-g3", "-Wall", "-Wextra",
    ...
    // arguments/output.
    {
        ...
        command_objects_push_back(&expected_list, &(CommandObject){
                                                      ...
                                                      .output    = SL("app"),
                                                      .file      = SL("main.c"),
                                                      .arguments = SLIST(&a, "gcc", "-o", "app", "main.c"),
                                                  });
        command_objects_push_back(&expected_list, &(CommandObject){
                                                      .ok        = 1,
                                                      .output    = SL("app"),
                                                      .file      = SL("mathlib.c"),
                                                      .arguments = SLIST(&a, "gcc", "-o", "app", "mathlib.c"),
                                                  });
        command_objects_push_back(&expected_list, &(CommandObject){
                                                      .ok        = 1,
                                                      .output    = SL("app"),
                                                      .file      = SL("strlib.c"),
                                                      .arguments = SLIST(&a, "gcc", "-o", "app", "strlib.c"),
                                                  });
        run_test_shell_parse_line(a, line, expected_list);
    }
    ...
    // (Taken from STM32 cubemx generated makefile)
    {
        ...
        command_objects_push_back(
            ...
            &(CommandObject){
                ...
                .arguments =
                    SLIST(&a, "arm-none-eabi-gcc", "-x", "assembler-with-cpp", "-c", "-mcpu=cortex-m4", "-mthumb",
    ...
    // NOTE: two sources here (header.h, main.c) → two CommandObject entries.
    {
        ...
        command_objects_push_back(&expected_list, &(CommandObject){
                                                      ...
                                                      .file      = SL("header.h"),
                                                      .output    = SL(""),
                                                      .arguments = SLIST(&a, "gcc", "-x", "c", "header.h", "-x", "none"),
                                                  });
        command_objects_push_back(&expected_list, &(CommandObject){
                                                      .ok        = 1,
                                                      .file      = SL("main.c"),
                                                      .output    = SL(""),
                                                      .arguments = SLIST(&a, "gcc", "-x", "c", "-x", "none", "main.c"),
                                                  });
        run_test_shell_parse_line(a, line, expected_list);
    }
            ...
            &(CommandObject){
                ...
                .arguments =
                    SLIST(&a, "g++", "-std=c++11", "-DHAVE_CONFIG_H", "-I.", "-I../src/config", "-U_FORTIFY_SOURCE",
                          "-D_FORTIFY_SOURCE=2", "-I./obj", "-I./secp256k1/include", "-DBUILD_BITCOIN_INTERNAL",
                          "-DHAVE_BUILD_INFO", "-D__STDC_FORMAT_MACROS", "-Wstack-protector", "-fstack-protector-all", "-fPIE",
    ...
    {
        Str               line          = SL("gcc -o make2compdb.exe .\\w64devkit\\src\\make2compdb.c");
        CommandObjectList expected_list = {0};
        command_objects_push_back(&expected_list,
                                  &(CommandObject){
                                      .ok        = 1,
                                      .file      = SL("./w64devkit/src/make2compdb.c"),
                                      .output    = SL("make2compdb.exe"),
                                      .arguments = SLIST(&a, "gcc", "-o", "make2compdb.exe", "./w64devkit/src/make2compdb.c"),
                                  });
        run_test_shell_parse_line(a, line, expected_list);
    }

    // `.output` should be escaped like `.arguments`
    {
        Str               line          = SL("gcc -c x.c -o 'my out.o'");
        CommandObjectList expected_list = {0};
        command_objects_push_back(&expected_list, &(CommandObject){
                                                      .ok        = 1,
                                                      .file      = SL("x.c"),
                                                      .output    = SL("my out.o"),
                                                      .arguments = SLIST(&a, "gcc", "-c", "x.c", "-o", "my out.o"),
                                                  });
        run_test_shell_parse_line(a, line, expected_list);
    }

    // -o flag should be ignored
    {
        Str               line          = SL("gcc x.c -o");
        CommandObjectList expected_list = {0};
        command_objects_push_back(&expected_list, &(CommandObject){
                                                      .ok        = 1,
                                                      .file      = SL("x.c"),
                                                      .arguments = SLIST(&a, "gcc", "x.c", "-o"),
                                                  });
        run_test_shell_parse_line(a, line, expected_list);
    }
}
...
static Str os_cwd_read(Arena *perm)
{
    ...
    Str converted = utf8_from_utf16(perm, dir16);
    return str_normalize_path(converted);
}
...
void mainCRTStartup(void)
{
    ...
    if (!exit_code && win_bufstream_stdout->stream.err) {
        exit_code = 1;
    }

    os_stream_flush(win_bufstream_stderr);
    if (!exit_code && win_bufstream_stderr->stream.err) {
        exit_code = 1;
    }
...
int main(int argc, char **argv)
{
    ...
    if (!exit_code && std_out.err) {
        exit_code = 1;
    }

    if (!exit_code && std_err.err) {
        exit_code = 1;
    }
    _exit(exit_code);
    unreachable();
